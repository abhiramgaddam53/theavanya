...
function CompleteBookingContent() {
  ...
  // Update your date formatting (add these near the top with other variables)
  ...
  const handleConfirmBooking = async () => {
    ...
    setIsLoading(true);
    setIsBooking(true);  // 🆕 Show booking preloader
    ...
    try {
      ...
      if (data.ok) {
        setBookingData({
          ...
          roomType: roomName,
          roomNo: "TBD",
          checkIn: checkInFormatted,     // ✅ Proper format
          checkOut: checkOutFormatted,
          total: priceString,
        });
        setShowConfirmation(true);
        // Remove alert - popup handles success
      } else {
      ...
      setIsLoading(false);
      setIsBooking(false);  // 🆕 Hide preloader
  ...
  // Update your useEffect for proper scroll lock
  // FIXED useEffect - Allows popup scroll while locking background
  // SIMPLIFIED & BULLETPROOF Scroll Fix
  useEffect(() => {
    if (showConfirmation) {
  ...
  return (
    <> {isLoading && (
      <PreloaderPayment isBooking={isBooking} />
    )}
      ...
      {showConfirmation && bookingData && (
        <>
          ...
          >
            {/* Confirmation Popup - Exact Email Template Layout */}
            <div
              ...
              onClick={(e) => e.stopPropagation()} style={{ borderRadius: '8px' }}
            >

              <button
                onClick={closeConfirmation} style={{ fontSize: '35px' }} title="Go Back To Home"  // ✅ Native tooltip (no tooltip prop needed)
              ...
              <div className="bg-gradient-to-r from-amber-50 to-orange-50/50 border-b border-amber-200/50 p-6 pt-10 pb-8 relative">
                <img
                  src="/logos/Black.png"
                  alt="Avanya Hotel"
                  className="absolute top-4 left-6 w-28 h-auto img-logo-class"
                />
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
                  <div className="space-y-4 pt-2">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        <div className="text-xs uppercase tracking-wide font-bold text-gray-500 mb-1">Name</div>
                        <div className="font-semibold text-gray-900">{bookingData.guestName}</div>
                      </div>
                      <div>
                        <div className="text-xs uppercase tracking-wide font-bold text-gray-500 mb-1">Room Type</div>
                        <div className="font-semibold text-gray-900">{bookingData.roomType}</div>
                      </div>
                      <div>
                        <div className="text-xs uppercase tracking-wide font-bold text-gray-500 mb-1">Check-in</div>
                        <div className="font-semibold text-gray-900">{bookingData.checkIn}</div>
                      </div>
                      <div>
                        <div className="text-xs uppercase tracking-wide font-bold text-gray-500 mb-1">Check-out</div>
                        <div className="font-semibold text-gray-900">{bookingData.checkOut}</div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4 pt-4 border-t border-gray-200">
                      <div>
                        <div className="text-xs uppercase tracking-wide font-bold text-gray-500 mb-1">Number of Guests</div>
                        <div className="font-semibold text-gray-900">1 Adult</div>
                      </div>
                      <div className="text-right">
                        <div className="text-xs uppercase tracking-wide font-bold text-gray-500 mb-1">Amount</div>
                        <div className="text-2xl font-bold text-gray-900">{bookingData.total}</div>
                      </div>
                    </div>
                  </div>
                  <div className="relative">
                    ...
                    />
                    <div className="absolute top-3 right-3 bg-white/90 px-3 py-1 rounded-full text-xs font-bold text-gray-700 shadow-sm">
                      {villaName}
                    </div>
                  </div>
              ...
              <div className="p-6 bg-white border-b border-gray-100">
                ...
                <div className="text-sm text-gray-700 space-y-1">
                  <div className="font-semibold">Avanya Resort</div>
